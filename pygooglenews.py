...
CREDIT: https://github.com/kotartemiy/pygooglenews
...
class GoogleNews:
    ...
    def __add_sub_articles(self, entries, political=False):
        for i, val in enumerate(entries):
            keys = entries[i].keys()

            if political and not isPolitical(entries[i]['title']): continue
            # print(entries[i]['source']['url'].lower())
            if "youtube" in entries[i]['source']['url'].lower():
                # print(entries[i]['source'])
                continue
    ...
    def topic_headlines(self, topic: str, proxies=None, scraping_bee=None, political=True):
        ...
        d['entries'] = self.__add_sub_articles(d['entries'], political)
        if len(d['entries']) > 0:
            return d
        else:
